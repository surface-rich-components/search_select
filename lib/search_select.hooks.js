...
const SelectHook = {
  ...
  mounted() {
    const { filterTarget, filterChange, filterDebounce } = this.el.dataset;
    ...
    let filterChangeEvent = (val) => this.pushEventTo(filterTarget, filterChange, val);
    if (filterDebounce && +filterDebounce > 0) {
      filterChangeEvent = debounce(filterChangeEvent, filterDebounce);
    }

    this.input.addEventListener("input", (event) => {
      filterChangeEvent(event.target.value);
      event.preventDefault();
      event.stopPropagation();
    });
  ...
  selectItem(itemElement) {
    const { selectTarget, selectChange } = this.el.dataset;
    const selectedValue = (itemElement && itemElement.dataset.selectValue) || null;
    this.hidden.value = selectedValue;
    this.hidden.dispatchEvent(new Event("input", { bubbles: true }));
    this.pushEventTo(selectTarget, selectChange, selectedValue);
    this.hideList();
  },
  selectPrev() {
